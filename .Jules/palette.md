...
## 2026-04-10 - Elevate critical security warnings with alert blocks
**Learning:** Security reporting instructions are among the most critical pieces of information in a repository. Using a `> [!WARNING]` alert block instead of simple bold text creates a strong visual anchor that is harder for users to miss and is more accessible to screen readers that recognize semantic callouts.
**Action:** Always use semantic GitHub-native alert blocks for high-stakes instructions like security reporting or destructive action warnings.
