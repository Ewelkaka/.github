...
## 2026-06-15 - Localize policy links and use alert blocks for enforcement visibility
**Learning:** Forcing users to leave the repository to read critical policies like the Code of Conduct increases friction and risks losing context. Localizing these links and using `> [!IMPORTANT]` alert blocks for reporting instructions makes enforcement processes more transparent and accessible, directly improving the contributor UX.
**Action:** Update global footer or contribution guides to point to local policy files and highlight reporting mechanisms using semantic alert blocks.
