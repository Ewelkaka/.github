...
# Optimization: Pre-instantiate mock result object at module scope to avoid
# defining classes and allocating dictionary objects repeatedly per test run.
class _MockResult:
...
class TestSetUpClassOptimization(unittest.TestCase):
    ...
    def test_meta_suite_loads_suites_in_setUpClass(self):
        """TestRefactoredSuitesStillPass should preload test suite IDs in setUpClass."""
        TestRefactoredSuitesStillPass.setUpClass()
        self.assertTrue(hasattr(TestRefactoredSuitesStillPass, "pr_accessibility_ids"))
...
class TestRefactoredSuitesStillPass(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Optimization: Precompute test IDs as tuples in setUpClass to avoid
        # re-traversing the TestSuite tree with _get_test_cases during test execution.
        cls.pr_accessibility_ids = tuple(t.id() for t in _get_test_cases(cls.pr_accessibility_suite))
        cls.readme_ux_ids = tuple(t.id() for t in _get_test_cases(cls.readme_ux_suite))
        cls.palette_ux_ids = tuple(t.id() for t in _get_test_cases(cls.palette_ux_suite))
        cls.contributing_ux_ids = tuple(t.id() for t in _get_test_cases(cls.contributing_ux_suite))
        cls.security_ux_ids = tuple(t.id() for t in _get_test_cases(cls.security_ux_suite))
    ...
    def test_pr_accessibility_suite_passes(self):
        result = self._run_module_suite(self.pr_accessibility_suite, self.pr_accessibility_ids)
        self.assertTrue(
    ...
    def test_readme_ux_suite_passes(self):
        result = self._run_module_suite(self.readme_ux_suite, self.readme_ux_ids)
        self.assertTrue(
            result.wasSuccessful(),
    ...
    def test_palette_ux_suite_passes(self):
        result = self._run_module_suite(self.palette_ux_suite, self.palette_ux_ids)
        self.assertTrue(
            result.wasSuccessful(),
    ...
    def test_contributing_ux_suite_passes(self):
        result = self._run_module_suite(self.contributing_ux_suite, self.contributing_ux_ids)
        self.assertTrue(
            result.wasSuccessful(),
    ...
    def test_security_ux_suite_passes(self):
        result = self._run_module_suite(self.security_ux_suite, self.security_ux_ids)
        self.assertTrue(
            result.wasSuccessful(),
    ...
    def test_coc_ux_suite_passes(self):
        result = self._run_module_suite(self.coc_ux_suite, self.coc_ux_ids)
        self.assertTrue(
            result.wasSuccessful(),
    ...
    def test_bolt_journal_suite_passes(self):
        result = self._run_module_suite(self.bolt_journal_suite, self.bolt_journal_ids)
        self.assertTrue(
            result.wasSuccessful(),
