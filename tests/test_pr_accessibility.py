...
# Pre-compile regular expressions at the module level to avoid redundant
# compilation during test execution.
EMPTY_ALT_RE = re.compile(r'<img\s[^>]*alt\s*=\s*["\']["\']')
WHITESPACE_ALT_RE = re.compile(r'<img\s[^>]*alt\s*=\s*["\'](\s+)["\']')
IMG_TAG_PRESENT_RE = re.compile(r"<img\s")
IMG_TAGS_RE = re.compile(r"<img\b[^>]*>", re.IGNORECASE)
ALT_MATCH_RE = re.compile(r'\balt\s*=\s*["\']([^"\']*)["\']', re.IGNORECASE)
...
class TestProfileReadmeAltText(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Baseline: 6 openat calls -> Optimized: 1 openat call.
    ...
    def test_img_alt_is_not_empty(self):
        ...
        empty_alt = EMPTY_ALT_RE.search(self.content)
        self.assertIsNone(
            empty_alt,
    ...
    def test_img_alt_not_whitespace_only(self):
        ...
        whitespace_alt = WHITESPACE_ALT_RE.search(self.content)
        self.assertIsNone(
            whitespace_alt,
    ...
    def test_all_img_tags_have_nonempty_alt(self):
        ...
        """
        img_tags = IMG_TAGS_RE.findall(self.content)
        for tag in img_tags:
            alt_match = ALT_MATCH_RE.search(tag)
...
class TestPaletteMarkdown(unittest.TestCase):
    ...
    def setUpClass(cls):
        # Optimization: Read file once for all tests in this class to reduce I/O.
        # Baseline: 10 openat calls -> Optimized: 1 openat call.
