...
class TestProfileReadmeAltText(TrackingTestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # Optimization: Read file once per class using _read_cached to eliminate redundant openat calls.
    ...
    def test_img_attributes_use_double_quotes(self):
        """All HTML attributes on <img> tags must be enclosed in double quotes for XHTML/HTML standard compliance."""
        self.assertIn(
            ...
            self.content,
            "Expected <img> tag attributes in profile/README.md to be enclosed in double quotes.",
        )
...
class TestPaletteMarkdown(TrackingTestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # Optimization: Read file once per class using _read_cached.
...
class TestContributingDiscoverability(TrackingTestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.content = _read_cached(CONTRIBUTING_MD)
