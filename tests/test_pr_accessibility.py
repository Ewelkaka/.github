...
class TestProfileReadmeAltText(TrackingTestCase):
    ...
    def setUpClass(cls):
        ...
        # Optimization: Read file once per class instead of once per test to reduce I/O overhead.
        # Reduces openat() calls for profile/README.md from 6 to 1.
        cls.content = _read(PROFILE_README)
        # Optimization: Read file once per class to reduce I/O (Bolt ⚡)
        cls.content = _read(PROFILE_README)
        # Optimization: Read file once for all tests in this class
        cls.content = _read(PROFILE_README)
        # Optimization: Read file once per class instead of once per test method.
        # Reduces openat() calls from 19 to 3 across the test suite.
        cls.content = _read(PROFILE_README)
        cls.content = _read(PROFILE_README)
        # Optimization: Read file once per class instead of once per test method.
        # Reduces openat() calls from O(N_tests) to O(1).
        cls.content = _read(PROFILE_README)
        # Optimization: Read file content once per class instead of per test
...
class TestPaletteMarkdown(TrackingTestCase):
    ...
    def setUpClass(cls):
        ...
        # Optimization: Read file once per class instead of once per test to reduce I/O overhead.
        # Reduces openat() calls for .Jules/palette.md from 10 to 1.
        cls.content = _read(PALETTE_MD)
        # Optimization: Read file once per class to reduce I/O (Bolt ⚡)
        cls.content = _read(PALETTE_MD)
        # Optimization: Read file once for all tests in this class
...
class TestCodeOfConductUX(TrackingTestCase):
    ...
    def test_contributing_coc_alert_block(self):
        ...
        self.assertIn(
            ...
            content,
            "Expected alert block content not found in CONTRIBUTING.md.",
        )
    ...
    def test_contributing_contains_alert_block(self):
        ...
        self.assertRegex(
            ...
            "The Code of Conduct disclaimer in CONTRIBUTING.md must be highlighted in a > [!IMPORTANT] alert block.",
        )
