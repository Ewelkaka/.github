...
IMG_TAG_START_RE = re.compile(r"<img\s")
ALL_IMG_TAGS_RE = re.compile(r"<img\b[^>]*>", re.IGNORECASE)
...
class TestProfileReadmeAltText(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        """Read the profile README content once for all tests in this class."""
    ...
    def test_all_img_tags_have_nonempty_alt(self):
        ...
        """
        img_tags = ALL_IMG_TAGS_RE.findall(self.content)
        for tag in img_tags:
...
class TestPaletteMarkdown(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        """Read the palette file content once for all tests in this class."""
