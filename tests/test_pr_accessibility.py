...
EMPTY_ALT_PATTERN = re.compile(r'<img\s[^>]*alt\s*=\s*["\']["\']')
WHITESPACE_ALT_PATTERN = re.compile(r'<img\s[^>]*alt\s*=\s*["\'](\s+)["\']')
...
class TestProfileReadmeAltText(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # Using setUpClass to read file only once for all tests in this class (Bolt optimization)
    ...
    def test_img_alt_is_not_empty(self):
        ...
        empty_alt = EMPTY_ALT_PATTERN.search(self.content)
        self.assertIsNone(
            empty_alt,
    ...
    def test_img_alt_not_whitespace_only(self):
        ...
        whitespace_alt = WHITESPACE_ALT_PATTERN.search(self.content)
        self.assertIsNone(
            whitespace_alt,
    ...
    def test_all_img_tags_have_nonempty_alt(self):
        ...
        """
        img_tags = IMG_TAG_PATTERN.findall(self.content)
        for tag in img_tags:
            alt_match = ALT_ATTR_PATTERN.search(tag)
