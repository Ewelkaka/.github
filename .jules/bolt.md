...
## 2026-07-17 - Comprehensive TrackingTestCase Inheritance Bypasses Meta-Test Suite Execution
**Learning:** In meta-test runners using global test execution tracking (`_PASSED_TESTS`), if any test class inherits directly from `unittest.TestCase` rather than `TrackingTestCase`, its test IDs are omitted from `_PASSED_TESTS`. This causes `all(test.id() in _PASSED_TESTS ...)` in meta-test suites to return `False`, forcing the runner to re-execute entire test suites and re-open static files.
**Action:** Ensure all test classes inherit from `TrackingTestCase` across all test modules so that their execution is recorded, enabling meta-test runners to short-circuit in O(1) time without re-running tests.
